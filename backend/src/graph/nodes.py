"""LangGraph nodes for the compliance audit pipeline."""
...
def index_video_node(state: VideoAuditState) -> Dict[str, Any]:
    """Downloads YouTube video, indexes via Azure Video Indexer, returns transcript and OCR."""
    ...
    try:
        ...
        logger.info(f"[Indexer] Upload complete. Azure ID: {azure_video_id}")
...
def audit_content_node(state: VideoAuditState) -> Dict[str, Any]:
    """Runs RAG against the compliance knowledge base and audits transcript via LLM."""
    logger.info("[Auditor] Querying knowledge base and LLM.")

    transcript = state.get("transcript", "")

    if not transcript:
        logger.warning("[Auditor] No transcript found. Skipping audit.")
        return {
            "final_status": "FAIL",
            "final_report": "Audit skipped — no transcript available from video processing."
        }
