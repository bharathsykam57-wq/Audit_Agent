"""One-time script to index compliance PDFs into Azure AI Search."""
...
logger = logging.getLogger("audit-agent-indexer")


def index_docs():
    """Loads PDFs from backend/data, chunks them, and uploads vectors to Azure AI Search."""
    ...
    logger.info(f"  EMBEDDING_DEPLOYMENT:       {os.getenv('AZURE_OPENAI_EMBEDDING_DEPLOYMENT')}")
    ...
    if all_splits:
        try:
            ...
            logger.error(f"Upload to Azure Search failed: {e}")
    else:
